        ...
        <div class="card-body">
            <form action="{{ route('jugadores.update', $jugador->id) }}" method="POST">
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" name="nombre" id="nombre" class="form-control @error('nombre') is-invalid @enderror" value="{{ old('nombre', $jugador->nombre) }}" required>
                    @error('nombre')
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="number" name="edad" id="edad" class="form-control @error('edad') is-invalid @enderror" value="{{ old('edad', $jugador->edad) }}" required min="16">
                    @error('edad')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group mb-3">
                    ...
                    <select name="equipo_id" id="equipo_id" class="form-control @error('equipo_id') is-invalid @enderror" required>
                        ...
                        @foreach($equipos as $equipo)
                            <option value="{{ $equipo->id }}" {{ old('equipo_id', $jugador->equipo_id) == $equipo->id ? 'selected' : '' }}>
                                {{ $equipo->nombre }}
                            </option>
                        @endforeach
                    </select>
